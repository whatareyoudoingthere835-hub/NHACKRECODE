...
public class MainMenuScreen extends Screen {
    ...
    public void render(@NotNull DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        FontRenderers.thglitchBig.drawCenteredString(context.getMatrices(), "NIGGAHACK", (int) (halfOfWidth), (int) (halfOfHeight - 120), new Color(255, 255, 255, hoveredLogo ? 230 : 180).getRGB());
        ...
        onlineText:
        {
            String onlineUsers = String.format("online: %s%s", Formatting.DARK_GREEN, Managers.TELEMETRY.getOnlineCount());
        ...
        int offsetY = 10;
        int maxLines = Math.min(5, ThunderUtility.changeLog.size());
        int startIndex = Math.max(0, ThunderUtility.changeLog.size() - maxLines);

        for (int i = startIndex; i < ThunderUtility.changeLog.size(); i++) {
            String change = ThunderUtility.changeLog.get(i);
            ...
            offsetY += 8; // Уменьшили с 10 до 8 для компактности
        }
