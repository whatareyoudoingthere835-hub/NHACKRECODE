...
public final class ThunderUtility {
    ...
    public static void parseCommits() {
        try {
            // Сырая ссылка на твой пастбин
            URL url = new URL("https://pastebin.com/raw/v5WcmZNL");
            BufferedReader in = new BufferedReader(new InputStreamReader(url.openStream(), StandardCharsets.UTF_8));

            changeLog.add("NIGGAHACK v1.0 ");
            changeLog.add("\n");

            String inputLine;
            // Просто читает всё, что написано у тебя на сайте, и добавляет в меню
            while ((inputLine = in.readLine()) != null) {
                changeLog.add(inputLine);
            }
            in.close();
        } catch (Exception e) {
            changeLog.add("Ошибка загрузки NIGGAHACK лога.");
            e.printStackTrace();
        }
    }
}