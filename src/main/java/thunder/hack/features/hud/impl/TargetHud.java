...
public class TargetHud extends HudElement {
    ...
    // Настройки вспышки хп (хил/дамаг)
    private final Setting<Boolean> hpFlash = new Setting<>("HPFlash", true);
    private final Setting<Integer> hpFlashDuration = new Setting<>("FlashDurationMs", 500, 100, 2000, v -> hpFlash.getValue());
    ...
    /**
     * Отслеживает изменение здоровья цели и запускает вспышку
     * зелёного (хил) или красного (дамаг) цвета.
     */
    private void updateHealthFlash(float currentHealth) {
        if (!hpFlash.getValue()) return;

        if (target != lastHealthTarget) {
            lastHealthTarget = target;
        ...
        if (lastHealth != -1f) {
            ...
            if (Math.abs(diff) > 0.05f) {
                flashColor = diff > 0 ? Color.GREEN.getRGB() : Color.RED.getRGB();
    ...
    /**
     * Возвращает цвет для отрисовки числа хп с учётом вспышки хил/дамаг.
     */
    private int getHpTextColor(float opacityFactor) {
        if (hpFlash.getValue()) {
            long elapsed = System.currentTimeMillis() - flashStart;
            long duration = hpFlashDuration.getValue();
            if (elapsed < duration) {
                float progress = MathUtility.clamp(elapsed / (float) duration, 0f, 1f);
                Color from = new Color(flashColor);
                Color to = new Color(Colors.WHITE);
                int r = (int) MathUtility.clamp(from.getRed() + (to.getRed() - from.getRed()) * progress, 0, 255);
                int g = (int) MathUtility.clamp(from.getGreen() + (to.getGreen() - from.getGreen()) * progress, 0, 255);
                int b = (int) MathUtility.clamp(from.getBlue() + (to.getBlue() - from.getBlue()) * progress, 0, 255);
                int blended = new Color(r, g, b).getRGB();
                return Render2DEngine.applyOpacity(blended, opacityFactor);
            }
        }
        return Render2DEngine.applyOpacity(Colors.WHITE, opacityFactor);
    }
    ...
    private void renderNurik(DrawContext context, float health, float animationFactor) {
        ...
        FontRenderers.sf_bold.drawCenteredString(context.getMatrices(), hpMode.getValue() == HPmodeEn.HP ? String.valueOf(Math.round(10.0 * getHealth()) / 10.0) : (((Math.round(10.0 * getHealth()) / 10.0) / 20f) * 100 + "%"), getPosX() + 92f, getPosY() + 35f,
                getHpTextColor(animationFactor));
        //
    ...
    private void renderMiniNurik(DrawContext context, float health, float animationFactor) {
        ...
        FontRenderers.sf_bold_mini.drawCenteredString(context.getMatrices(), hpMode.getValue() == HPmodeEn.HP ? String.valueOf(Math.round(10.0 * getHealth()) / 10.0) : (((Math.round(10.0 * getHealth()) / 10.0) / 20f) * 100 + "%"), getPosX() + 65, getPosY() + 27f, getHpTextColor(animationFactor));
        //
    ...
    private void renderThunderHack(DrawContext context, float health, float animationFactor) {
        ...
        FontRenderers.sf_bold.drawCenteredString(context.getMatrices(), hpMode.getValue() == HPmodeEn.HP ? String.valueOf(Math.round(10.0 * getHealth()) / 10.0) : (int) (((Math.round(10.0 * health) / 10.0) / 20f) * 100) + "%", getPosX() + 102, getPosY() + 24f, getHpTextColor(animationFactor));
