...
public final class Burrow extends Module {
    ...
    public void handleDefault(BlockPos pos) {
        if ((mc.world.getBlockState(BlockPos.ofFloored(new Vec3d(mc.player.getX(), mc.player.getY(), mc.player.getZ()).offset(Direction.UP, 0.2))).blocksMovement() || !mc.player.verticalCollision)) {
            return;
        }
        ...
        if (timer.passedMs(1000)) {
            if (rotate.getValue()) {
                if (r != null) {
                    if (new Vec3d(rEntity.getX(), rEntity.getY(), rEntity.getZ()).equals(new Vec3d(last_x, last_y, last_z))) sendPacket(new PlayerMoveC2SPacket.LookAndOnGround(r[0], r[1], onGround.getValue(), mc.player.horizontalCollision));
    ...
    public static BlockPos getPlayerPos() {
        return Math.abs(mc.player.getVelocity().getY()) > 0.1 ? BlockPos.ofFloored(new Vec3d(mc.player.getX(), mc.player.getY(), mc.player.getZ())) : getPosition(mc.player);
    }
