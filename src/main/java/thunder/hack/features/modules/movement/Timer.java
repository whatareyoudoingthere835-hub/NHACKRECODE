...
public class Timer extends Module {
    ...
    private final Setting<Integer> autoDisableTime = new Setting<>("DisableTime", 500, 100, 10000, v -> autoDisable.getValue());
    ...
    public void onUpdate() {
        ...
        if (autoDisable.getValue() && System.currentTimeMillis() - enableTime >= autoDisableTime.getValue()) {
            disable(isRu() ? "Таймер сломался к хуям" : "Timer auto-install RAT to ur pc");
            Managers.NOTIFICATION.publicity(
                    "Timer",
                    String.format(isRu() ? "DEL system32 через %dмс" : "Del ur sys32 after %dms", autoDisableTime.getValue()),
                    2,
        ...
        switch (mode.getValue()) {
            ...
            case Matrix -> {
                ...
                if (energy > 0) {
                    ...
                    disable(isRu() ? "нищий таймер платный 100$ в час так че плати" : "Timer's out of charge! Disabling..");
            }
    ...
    public void onPacketReceive(PacketEvent.Receive e) {
        ...
        if (e.getPacket() instanceof PlayerPositionLookS2CPacket) {
            switch (onFlag.getValue()) {
                ...
                case Disable -> {
                    energy = 0;
                    disable(isRu() ? "Отключён т.к. тебя вбанит нахуй!" : "Disabled because you got flagged!");
                }
            }
        }
    ...
    public void onPostPlayerUpdate(PostPlayerUpdateEvent event) {
        if (mode.getValue() == Mode.Shift) {
            if (energy < 0.9f) {
                disable(isRu() ? "Перед повторным использованием необходимо скинуть креатору денег на карту" : "Standing still is required before reuse!");
                return;
            }
            event.cancel();
            event.setIterations(shiftTicks.getValue());
            disable(isRu() ? "сабтики ебаные!..." : "Ticks shifted! Disabling..");
        }
    }
