...
public class Speed extends Module {
    ...
    // Настройки для ElytraSpoof
    public final Setting<Boolean> autoJump = new Setting<>("AutoJump", true, v -> mode.is(Mode.ElytraSpoof));
    public final Setting<Float> elytraSpoofSpeed = new Setting<>("SpoofSpeed", 0.5f, 0.1f, 2f, v -> mode.is(Mode.ElytraSpoof));
    public final Setting<Integer> elytraSlot = new Setting<>("ElytraSlot", 1, 1, 9, v -> mode.is(Mode.ElytraSpoof));
    ...
    private thunder.hack.utility.Timer elytraSpoofTimer = new thunder.hack.utility.Timer();
    private boolean wasFlyingSpoof = false;

    public enum Mode {
        StrictStrafe, MatrixJB, NCP, ElytraLowHop, MatrixDamage, GrimEntity, GrimEntity2, FireWork, Vanilla, GrimIce, GrimCombo, ElytraSpoof
    }

    @Override
    public void onDisable() {
        ThunderHack.TICK_TIMER = 1f;
        if (mode.is(Mode.ElytraSpoof) && wasFlyingSpoof) {
    ...
    public void onSync(EventSync e) {
        ...
        // ElytraSpoof логика
        if (mode.is(Mode.ElytraSpoof)) {
            handleElytraSpoof();
        }
    }

    private void handleElytraSpoof() {
        ...
        // Проверяем наличие элитры в хотбаре
        SearchInvResult elytraResult = InventoryUtility.findItemInHotBar(Items.ELYTRA);
        ...
        if (elytraSpoofTimer.passedMs(100)) {
            ...
            // Отправляем пакет что мы начали полет
            sendPacket(new ClientCommandC2SPacket(mc.player, ClientCommandC2SPacket.Mode.START_FALL_FLYING));
        ...
        if (mc.player.fallDistance > 0 && !mc.player.isOnGround()) {
            double[] motion = MovementUtility.forward(elytraSpoofSpeed.getValue());
    ...
    public void onMove(EventMove event) {
        ...
        if (MovementUtility.isMoving()) {
            ...
            float currentSpeed = mode.getValue() == Mode.NCP && thunder.hack.utility.player.InputUtility.forward() <= 0 && prevForward > 0 ? Managers.PLAYER.currentPlayerSpeed * 0.66f : Managers.PLAYER.currentPlayerSpeed;
            ...
            double ncpSpeed = mode.getValue() == Mode.StrictStrafe || thunder.hack.utility.player.InputUtility.forward() < 1 ? 0.465 : 0.576;
            double ncpBypassSpeed = mode.getValue() == Mode.StrictStrafe || thunder.hack.utility.player.InputUtility.forward() < 1 ? 0.44 : 0.57;
            ...
            prevForward = thunder.hack.utility.player.InputUtility.forward();
        } else {
            ThunderHack.TICK_TIMER = 1f;
            event.setX(0);
            event.setZ(0);
